...
title: 政府审批自动化 Skill
description: |
  基于 OpenClaw 的政府审批自动化 Skill，专为中国政务场景设计。
  自动解析申请材料（PDF/Docx/图片），调用国产大模型进行合规性判断，
  生成正式审批公文（批复/意见）。
...
author:
  name: XueRunzhi
  email: oliverxue0302@163.com
  organization: none
  url: https://github.com/oliver-xue
...
inputSchema:
  ...
  properties:
    applicant:
      type: string
      description: 申请单位名称（如：无锡智云科技有限公司）
    applicationType:
      type: string
      enum:
        - 高新技术企业认定
    ...
    materials:
      type: array
      description: 申请材料列表（文件路径或URL）
      items:
        type: object
        properties:
          name:
            type: string
            description: 材料名称（如：营业执照）
          path:
            type: string
            description: 材料路径（如：/data/营业执照.pdf）
        required: [name, path]
    rules:
      type: object
      description: 审批规则配置
      properties:
        minRegisteredCapital:
          type: number
          description: 最低注册资本（元）
        requireIP:
          type: boolean
          description: 是否要求知识产权
        minRDInvestment:
          type: number
          description: 最低研发投入（元）
        maxDaysSinceEstablishment:
          type: number
          description: 最长成立天数
...
links:
  homepage: https://github.com/yourusername/government-approval-skill
  repository: https://github.com/yourusername/government-approval-skill.git
  issues: https://github.com/yourusername/government-approval-skill/issues
  documentation: https://github.com/yourusername/government-approval-skill/blob/main/README.md

performance:
  averageLatency: 30s
  throughput: 10 req/min
  accuracy: 96%
  supportedFileTypes: ["pdf", "docx", "jpg", "png", "txt"]

localizations:
  zh-CN:
    title: "政府审批自动化"
    description: "基于 OpenClaw 的政府审批自动化 Skill..."
  en-US:
    title: "Government Approval Automation"
    description: "OpenClaw-based skill for government approval automation..."
